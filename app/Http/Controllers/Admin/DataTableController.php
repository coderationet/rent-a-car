...
class DataTableController extends Controller
{
    ...
    public function __invoke(IndexDataTableGetRequest $request)
    {
        ...
        $route_name_prefix = request()->filled('route_name_prefix') ? request()->get('route_name_prefix') . '.' : 'admin.'.request()->get('table_name') . '.';

        $items = $items->orderBy($order_by, $order_dir)->offset($offset)->limit($limit)->get()->map(function ($item) use($column_names,$route_name_prefix) {
